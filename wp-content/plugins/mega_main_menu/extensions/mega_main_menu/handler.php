 ...
 */
	...
	function mmm_set_walkers ( $args ){
		...
		if ( ( is_array( $mega_menu_locations ) && in_array( $theme_location_safe_name, $mega_menu_locations ) ) || ( $indefinite_location_mode === true ) || ( $args['theme_location'] == 'mega_main_sidebar_menu' ) || ( $args['theme_location'] == 'mega_main_flyout_menu' ) ) {
			...
			// Default args
			$args['items_wrap'] = '<ul id="%1$s" class="%2$s" role="menubar" aria-label="' . $mega_main_menu->get_option( $theme_location_safe_name . '_mobile_label', 'Main Menu' ) . '">%3$s</ul>';
			$args['walker'] = new Mega_Main_Menu_Frontend_Walker;
			$args['container'] = false;
			$args['container_id'] = '';
			$args['container_class'] = '';
			$args['menu_id'] = 'main_ul-' . $theme_location_safe_name;
...
*/
			...
			foreach ( $mega_main_menu->get_option( $theme_location_safe_name . '_logo_positions', array( 'desktop', 'sticky', 'mobile', ) ) as $key => $value ) {
				if ( $value != 'is_checkbox' ) {
					$container_class[] = 'logo-' . $value;
				}
			}
			...
			$items_wrap .= mm_common::ntab(2) . '<div class="menu_inner" role="navigation">';
			$items_wrap .= mm_common::ntab(3) . '<span class="nav_logo">';
			if( $mega_main_menu->get_option( $theme_location_safe_name . '_logo_src' ) ) {
				$logo_link = $mega_main_menu->get_option( $theme_location_safe_name . '_logo_link' );
				$logo_link = ( !empty( $logo_link ) ? $logo_link : home_url() );
				$items_wrap .= mm_common::ntab(4) . '<a class="logo_link" href="' . $logo_link . '" title="' . get_bloginfo( 'name' ) . '">';
				$items_wrap .= mm_common::ntab(5) . '<img src="' . str_replace( array('%','$'), array('',''), $mega_main_menu->get_option( $theme_location_safe_name . '_logo_src' ) ) . '" alt="' . get_bloginfo( 'name' ) . '" />';
				$items_wrap .= mm_common::ntab(4) . '</a>';
			}
			$items_wrap .= mm_common::ntab(4) . '<span class="mobile_toggle" data-target="#' . $args['menu_id'] . '" aria-expanded="false" aria-controls="' . $args['menu_id'] . '">';
			...
			$items_wrap .= mm_common::ntab(5) . '</span><!-- class="mobile_button" -->';
			$items_wrap .= mm_common::ntab(4) . '</span><!-- /class="mobile_toggle" -->';
			$items_wrap .= mm_common::ntab(3) . '</span><!-- /class="nav_logo" -->';
	...
	function mmm_nav_search( $items, $args ) {
		...
		if( isset( $args->theme_location ) ) {
			...
			if( ( in_array( $args->theme_location, $mega_menu_locations) ) && is_array( $mega_main_menu->get_option( $args->theme_location . '_included_components' ) ) && in_array( 'search_box', $mega_main_menu->get_option( $args->theme_location . '_included_components' ) ) ) {
				...
				$searchform .= mm_common::ntab(2) . '<form method="get" class="mega_main_menu_searchform" action="' . esc_url( home_url( '/' ) ) . '">';
				$searchform .= mm_common::ntab(3) . '<i class="' . $icon . ' icosearch"></i>';
				$searchform .= mm_common::ntab(3) . '<input type="text" class="field" name="s" id="s" />';
				$searchform .= mm_common::ntab(3) . '<input type="submit" class="submit" name="submit" id="searchsubmit" value="' . esc_attr__( 'Search', 'mega_main_menu' ) . '" />';
