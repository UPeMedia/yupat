...
class PluginUpdateChecker {
	...
	public $check_period = 48; //How often to check for updates (in hours).
...
*/
		...
		//The plugin info should be at 'https://your-api.com/url/here/$slug/info.json'
...
class PluginInfo {
	...
	public static function fromJson($json, $triggerErrors = false){
		...
		if ( empty($apiResponse) || !is_object($apiResponse) ){
			if ( $triggerErrors ) {
				trigger_error(
					"Failed to parse plugin metadata. Try validating your .json file with https://jsonlint.com/",
					E_USER_NOTICE
				);
			}
			return null;
...
class PluginUpdate {
	...
	$current_class = $this;
	$siteurl = str_replace( array( 'http://', 'https://', '.', '/' ), array( '', '', '|', '+' ), get_option( 'siteurl', '' ) );
	...
	$UpdateChecker = new PluginUpdateChecker(
		'https://megamain.com/updates/index.php?purchase_code=' . $purchase_code . '&folder=' . $slug . '&url=' . $siteurl,
