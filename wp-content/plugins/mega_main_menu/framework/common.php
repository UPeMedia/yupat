...
/*
							if ( $terms_value->parent != 0 ) {
								$parent_obj = get_term( $terms_value->parent );
								if ( isset( $parent_obj->name ) ) {
									$terms_value->name = $parent_obj->name . ' &nbsp; -&gt; &nbsp; ' . $terms_value->name;
								}
							}
*/
							$out[ ucfirst( $single_post_type ) . ' &nbsp; -&gt; &nbsp; ' . $terms_value->name . ' (ID=' . $terms_value->term_id . ')' ] = $single_post_type . '/' . $terms_value->taxonomy . '=' . $terms_value->slug;
						}
					}
				}
				asort( $out );
...
*/
		 ...
		 * @return string value.
		 */
		public static function transpose_array ( $array = array(), $convert_stringnumbers_to_real_numbers = false ) {
...
//						$difference = $max_columns - $columns_number_in_row;
		 ...
		 * The function converts PHP array to the JSON string.
		 */
		public static function json_string_to_array ( $variable ) {
			$variable = json_encode( $variable );
			$variable = preg_replace( '/,"([\d.]*)"/i', ',$1', $variable );
			$variable = str_replace( '"', "'", $variable );
			return $variable;
